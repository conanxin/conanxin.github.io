const SITE_VERSION = "Phase 8 · Sort, Performance & Mobile · 2026-05-11";
...
return `<svg class="exhibit-svg-placeholder" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid slice" xmlns="http:
...
function getSortedWorks(works, sortMode) {
if (!works) return [];
const arr = [...works];
switch (sortMode) {
case 'artist_az':
return arr.sort((a, b) => a.artist.localeCompare(b.artist, 'zh'));
case 'title_az':
return arr.sort((a, b) => (a.title || '').localeCompare(b.title || '', 'zh'));
case 'recommended':
return arr.sort((a, b) => {
const aRec = (a.route_tags || []).includes('media_politics') ? 1 : 0;
const bRec = (b.route_tags || []).includes('media_politics') ? 1 : 0;
return bRec - aRec;
});
case 'chinese':
return arr.sort((a, b) => {
const aCN = (a.route_tags || []).includes('chinese_response') ? 1 : 0;
const bCN = (b.route_tags || []).includes('chinese_response') ? 1 : 0;
return bCN - aCN;
});
default:
return arr;
...
let works = window._allWorks || [];
works = getSortedWorks(works, sortMode);
window._currentWorks = works;
if (randomActive) {
works = works.filter(w => window._randomRouteIds.has(w.id));
} else {
if (activeArea !== 'all') {
works = works.filter(w => w.color_area === activeArea);
}
if (activeRoute !== 'all') {
works = works.filter(w => (w.route_tags || []).includes(activeRoute));
}
if (searchQ) {
works = works.filter(w => {
const searchable = [
w.title, w.title_en || '', w.artist, w.artist_en || '',
COLOR_NAMES[w.color_area] || '', (w.route_tags || []).join(' '),
(w.mood_tags || []).join(' ')
].join(' ').toLowerCase();
return searchable.includes(searchQ);
});
}
if (speedActive) {
const seen = new Set();
...
const visibleIds = new Set(works.map(w => w.id));
const cards = document.querySelectorAll('.exhibit-card');
let visible = 0;
cards.forEach(card => {
const workId = card.id.replace('work-', '');
card.style.display = visibleIds.has(workId) ? '' : 'none';
if (visibleIds.has(workId)) visible++;
});
updateFilterStatus();
if (searchQ) {
const countEl = document.getElementById('searchResultCount');
