/**
 * scene-data.js — CP-5F Immersive Mode
 * Six scene spatial mapping for 3D camera and audio state.
 * Each scene defines camera position/target and accent color.
 *
 * CP-5F Changes:
 * - Scene 02: diagonal layered composition
 * - Scene 03: overhead gallery angle
 * - Scene 04: pulled closer for hub identity
 * - Scene 05: low-angle upward shot
 * - Scene 06: central-axis perspective for portal depth
 */
...
window.CP_IMMERSIVE_SCENES = [
  ...
  },
  {
    ...
    // CP-5F: diagonal layered composition — from left-front angle
    camera: {
      desktop: { position: { x: -2, y: 3.5, z: 7 },  target: { x: 0, y: 2.0, z: 0 } },
      mobile:  { position: { x: -2, y: 3.0, z: 5.5 },  target: { x: 0, y: 1.8, z: 0 } }
    },
    accentColor: '#a08cf7',
    audioMood: 'chat',
  ...
  },
  {
    ...
    // CP-5F: overhead gallery angle — looking down at exhibition floor
    camera: {
      desktop: { position: { x: 0, y: 6, z: 9 },  target: { x: 0, y: 1.5, z: 0 } },
      mobile:  { position: { x: 0, y: 5, z: 7 },  target: { x: 0, y: 1.2, z: 0 } }
    },
    accentColor: '#7af7b8',
    audioMood: 'artifacts',
  ...
  },
  {
    ...
    // CP-5F: low-angle upward shot — dramatic tower composition
    camera: {
      desktop: { position: { x: -2, y: 1.5, z: -10 }, target: { x: 0, y: 4, z: -20 } },
      mobile:  { position: { x: -2, y: 1.2, z: -8 },  target: { x: 0, y: 3.5, z: -20 } }
    },
    accentColor: '#f78c7a',
    audioMood: 'tower',
  ...
  },
  {
    ...
    // CP-5F: central-axis perspective — portal depth emphasis
    camera: {
      desktop: { position: { x: 0, y: 3.5, z: -3 },  target: { x: 0, y: 2, z: -25 } },
      mobile:  { position: { x: 0, y: 3, z: -1 },    target: { x: 0, y: 2, z: -25 } }
    },
    accentColor: '#7ac8f7',
    audioMood: 'archive',
