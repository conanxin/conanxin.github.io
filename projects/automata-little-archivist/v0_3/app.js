...
function getProfileDisplacement(phase) {
  const t = phase / (2 * Math.PI); // 0 to 1

  switch (motion.profile) {
    case 'mechanical':
      return (1 - Math.cos(phase)) / 2; // standard sine

    case 'slow':
      // Smoothstep-like: slower at extremes
      const s = (1 - Math.cos(phase)) / 2;
      return s * s * (3 - 2 * s);

    case 'tired': {
      // Asymmetric: slow rise, pause at top, slower fall
    ...
    case 'sleeping': {
      // Very low amplitude, long pauses
      if (t < 0.6) return 0.1 + Math.sin(t / 0.6 * Math.PI) * 0.15;
      return 0.1;
    }

    case 'alert': {
      // Fast rise, brief pause, slow fall
...
function applyMotion() {
  ...
  const maxLift = 5.0; // mm from cam spec
...
function updateMotionInfo() {
  ...
  document.getElementById('mi-profile').textContent = profileNames[motion.profile] || motion.profile;
  document.getElementById('mi-phase').textContent = `${Math.round(motion.phaseDeg)}°`;
  document.getElementById('mi-follower').textContent = `${motion.followerDisp.toFixed(2)} mm`;
  document.getElementById('mi-body').textContent = `${motion.bodyOffset.toFixed(2)} mm`;
  document.getElementById('mi-status').textContent = motion.playing ? 'Playing' : 'Paused';
}
...
// ── Animation loop ──
function animateMotion(delta) {
  if (!motion.playing || currentMode !== 'motion') return;

  // Speed: 1 = very slow (~10 seconds per cycle), 5 = fast (~2 seconds)
  const cycleDuration = 12 / motion.speed; // seconds per 360°
...
function updateMotionPath() {
  ...
  motionLines.forEach(ml => {
    ...
    if (!mesh || !mesh.visible || !cfg.vertical_motion_scale) {
    ...
    // Draw vertical line showing motion range
